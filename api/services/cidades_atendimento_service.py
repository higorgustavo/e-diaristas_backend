...
def cadastrar_cidade(codigo_ibge, cidade, estado):
    return CidadesAtendimento.objects.get_or_create(codigo_ibge=codigo_ibge, defaults=dict(
        cidade=cidade, 
        estado=estado
    ))
...
def relacionar_cidade_diarista(usuario, cidades):
    ...
    for cidade in cidades.value:
        ...
        cidade_nova, create = cadastrar_cidade(cidade['codigo_ibge'], dados_api['nome'], dados_api['microrregiao']['mesorregiao']['UF']['sigla'])
